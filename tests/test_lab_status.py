...
class LabStatusTests(unittest.TestCase):
    def test_lab_status_summarizes_current_goal_state_without_completion(self):
        ...
        self.assertEqual(status["skin_dirs"]["candidate_count"], 1)
        self.assertIn("smoke_readiness", status)
        self.assertEqual(status["smoke_readiness"]["status"], "ready_to_run_tmuf_smoke")
        self.assertIn("record_tmuf_smoke_evidence", status["smoke_readiness"]["next_actions"])
        self.assertIn("commands", status["smoke_readiness"])
    ...
    def test_cli_summary_includes_smoke_kit_freshness(self):
        ...
        self.assertIn("smoke_readiness=", output)
